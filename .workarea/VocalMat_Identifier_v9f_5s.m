...
for minute_frame = 1:ceil(size(y1,1)/(60*250000)) %run through all the minute windows
    ...
    if minute_frame == 1 %this division only matters for the spectrogram calculation
       try
            y2 = y1(60*(minute_frame-1)*250000+1:(60*minute_frame+overlap)*250000); %Window size in seconds
        catch % in case this file is shorter than one minte
            y2 = y1(60*(minute_frame-1)*250000+1:end); %Window size in seconds
        end
    elseif minute_frame == ceil(size(y1,1)/(60*250000))
        y2 = y1((60*(minute_frame-1)-overlap)*250000+1:end);
    else
        if (60*minute_frame+overlap)*250000>size(y1,1)
            y2 = y1((60*(minute_frame-1)-overlap)*250000+1:end);
        else
            y2 = y1((60*(minute_frame-1)-overlap)*250000+1:(60*minute_frame+overlap)*250000);
        end
    end
    ...
    A = 10*log10(P);
    if minute_frame==1
        A = A(:,300:end); %Cut off the first 0.3s... usually there is a weird noise in the beggining.
        T = T(:,300:end);
    end
...
%             aux_median_stats = [aux_median_stats, [median(temp(end-5:end))  median_db-0.1*median_db]];
        ...
        %         median_stats(:,6) = median_stats(:,6)/0.9;
...
%         plot(t(maxx),y(maxx),'r*','markers',12)
        th_ratio = t(maxx);
    ...
    if pdf_filter==1
        cd(raiz)
        disp('Eliminating points with low intensity ')
