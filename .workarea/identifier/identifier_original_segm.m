...
segm_size = str2num(getenv('SEGMENT_SIZE')); %in minutes
segments = segm_size:segm_size:ceil(size(y1,1)/(60*250000));

if segments(end)<ceil(size(y1,1)/(60*250000))
    segments = [segments, ceil(size(y1,1)/(60*250000))];
end


for minute_frame = 1:size(segments,2) %run through all the minute windows
    disp(['Current minute: ' num2str(segments(minute_frame))])
    %     jump = 0;%3*5000000;
    clear A B y2 S F T P q vocal id F_orig grain
    
    if minute_frame == 1 %this division only matters for the spectrogram calculation
        try
            y2 = y1(60*(segments(minute_frame)-segm_size)*250000+1:(60*segments(minute_frame)+overlap)*250000); %Window size in seconds
        catch % in case this file is shorter than one minte
            y2 = y1(60*(segments(minute_frame)-segm_size)*250000+1:end); %Window size in seconds
        end
    elseif minute_frame == size(segments,2)
        y2 = y1((60*(segments(minute_frame-1))-overlap)*250000+1:end);
    else
        if (60*segments(minute_frame)+overlap)*250000>size(y1,1)
            y2 = y1((60*(segments(minute_frame)-segm_size)-overlap)*250000+1:end);
        else
            y2 = y1((60*(segments(minute_frame)-segm_size)-overlap)*250000+1:(60*segments(minute_frame)+overlap)*250000);
        end
    end
    ...
    A = 10*log10(P);
    if minute_frame==1
        A = A(:,600:end); %Cut off the first 0.18s... usually there is a weird noise in the beggining
        T = T(:,600:end);
    end
    ...
    if segments(minute_frame) == segm_size
        ...
        disp(['Interval: 0 to ' num2str(60*(segments(minute_frame)))])
    elseif minute_frame == size(segments,2)
        ...
        disp(['Interval: ' num2str(60*(segments(minute_frame-1))) ' to end' ])
    else
        ...
        lim_superior = find(T<=60*segments(minute_frame),1,'last');
        disp(['Interval: ' num2str(60*(segments(minute_frame)-segm_size)) ' to end ' num2str(60*segments(minute_frame))])
    end
...
%Just for test
median_db = repmat(median(A_total,2),1,size(A_total,2));
A2 = A_total - median_db;
...
for k=1:size(graindata_2,1)-1
    ...
    centroid_to_id = [centroid_to_id; [id,k, T_orig(time_vocal{id}(1)), graindata_2(k).Area]];
end
...
if size(time_vocal,2)>0
    ...
    if local_median == 1
        ...
        for k=1:size(time_vocal,2)
            ...
            try
                ...
                aux_median_stats = [aux_median_stats, prod(size(A_total(find(min_local_freq(k)==F_orig)-5:find(max_local_freq(k)==F_orig)+5,find(T_orig==time_vocal{k}(pos))-200 : find(T_orig==time_vocal{k}(pos)) + 200)))];
            catch
            ...
            elim_by_median = 0;
            if median(temp(end-5:end)) < 0.9*median_db %-0.1*median_db %it is a subtraction because the intensity is already negative. So it is increasing the
    ...
    save(['output_shorter_' vfilename],'T_orig','F_orig','time_vocal','freq_vocal','vfilename','intens_vocal','compare_bimodality','median_stats')
    
    if save_spectogram_background==1
        save(['output_' vfilename],'T_orig','F_orig','time_vocal','freq_vocal','vfilename','intens_vocal','median_stats','A_total','-v7.3')
    else
