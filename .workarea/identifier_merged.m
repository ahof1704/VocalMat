...
%%%% -- (X) TODO -------------------------------------------------------------------------------
%%%% -- A_total size calculation outside image-processing-for for possible better performance
%%%% -- vectorize?
%%%% -- change data structures to test for performance boost
%%%% -- clear unused variables to alleviate memory usage
%%%% --
%%%% -------------------------------------------------------------------------------------------
...
% -- if there are vocalizations, remove the ones that have less than 6 points
    ...
    save([datestr(now,'yyyy-mm-dd_HH-MM-SS-FFF') '_output_short_' vfilename], 'T_orig', 'F_orig', 'time_vocal', 'freq_vocal', 'vfilename', 'intens_vocal', 'compare_bimodality', 'median_stats')
