...
%              two_steps_count_bin = [ sum(noise_count <= bin_1(end)), sum(noise_count>=bin_2(1) & noise_count<=bin_2(end)), sum(noise_count>=bin_3(1) & noise_count<=bin_3(end)), sum(noise_count>=bin_4(1) & noise_count<=bin_4(end))];
%          end
%     
%     
...
%         end
        ...
        %     system(['R --slave --args' ' ' char(34) corr_table char(34) ' ' 'wavs "0.80" "5" < getClusterCenterUSV_pub.r']);
         ...
         disp('Number of vocalizations identified in each class:')
         disp(['stepup_bin1: ' num2str(size(stepup_count_bin,2))]);
         disp(['stepup: ' num2str(size(stepup_count_bin,2))]);
         disp(['stepup: ' num2str(size(stepup_count_bin,2))]);
         disp(['stepup: ' num2str(size(stepup_count_bin,2))]);
         disp(['stepup: ' num2str(size(stepup_count_bin,2))]);
         disp(['stepup: ' num2str(size(stepup_count_bin,2))]);
