...
%     model_class_DL size_spectrogram use_DL model_class_DL_RF bin_size use_RF
...
%         num2cell([max_below_50k_total(k), max_prom(k), max_prom2(k), median_dist_total(k), mean_dist_total(k), mean_pks_valley(k), corr_yy2_yy3(k), corr_yy2_yy4(k)])]; % New classifier results
    table_out(k,:) = [num2cell(aux1)]; % New classifier results
...
names_out = [{'name_vocal'},{'start_time'}];
...
if use_RF==1
    ...
    table_total_output = [table_aux(:,1:2)];
end
