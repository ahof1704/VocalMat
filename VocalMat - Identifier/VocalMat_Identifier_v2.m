...
for Name = 2:size(list,1)
    ...
    if size(time_vocal,2)>0
        ...
        for k=1:size(time_vocal,2)
            ...
            try
                median_db = median(median(A(find(min_local_freq(k)==F_orig)-5:find(max_local_freq(k)==F_orig)+5,find(T_orig==time_vocal{k}(ceil(end/2)))-200 : find(T_orig==time_vocal{k}(ceil(end/2))) + 200))); %Calculating median in the freq range of identified vocalization
            catch
            ...
            if median(temp(end-5:end)) < median_db-0.25*median_db
