...
for Name = 1%size(list,1)
    ...
    for minute_frame = 3%1:size(y1,1)/(60*250000) %run through all the minute windows
        disp(['Current minute: ' num2str(minute_frame)])
        %     jump = 0;%3*5000000;
        clear A B y2 S F T P q vocal id F_orig grain
        y2 = y1((60*(minute_frame-1)+33)*250000+(60*(minute_frame-1)+34)*250000); %Window size in seconds
        nfft = 1024;
        nover = (128);
        window = hamming(256);
        %     db_threshold = -200;
