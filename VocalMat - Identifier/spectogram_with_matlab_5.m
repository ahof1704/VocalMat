...
for k = 1:size(T,2)-1
    ...
    if T(k+1)-T(k)> 0.003 %If >0.002s, it is a new vocalization
        id=id+1;
        time_vocal{id} = [];
        freq_vocal{id} = [];
        intens_vocal{id} = [];
...
for k=1:size(time_vocal,2)
   if  size(time_vocal{k},2) < 10 %|| max(freq_vocal{k})-min(freq_vocal{k}) > 45000
       time_vocal{k}=[];
       freq_vocal{k}=[];
       intens_vocal{k}=[];
   end
   ...
   if median(dist) > 800 %in general, when it is a real vocalization, the median is exaclty 244.1406!!
       time_vocal{k}=[];
       freq_vocal{k}=[];
       intens_vocal{k}=[];
   end
...
denoiseing=0;
for k=1:size(time_vocal,2)
    if (max(time_vocal{k})-min(time_vocal{k}))>0.140 %125ms
        denoiseing = 1;
