...
cd(vpathname);
list = dir('*.wav');
min_db = -115;%-110;%-107; %selec points >min_db
max_interval = 0.002; %if the distance between two successive points in time is >max_interval, it is new vocalization
minimum_size = 12;%10; %A valid vocalization must present >minimum_size valid points to be considered a vocalization
median_dist = 650; %600; If the median of the euclidean distance between succesive pair of points in a vocalization is >median_dist, then it is noise.
...
for k=1:size(time_vocal,2)
    if ~isempty(time_vocal{k}) && time_vocal{k}(1) > 309.2 
        k
    end
