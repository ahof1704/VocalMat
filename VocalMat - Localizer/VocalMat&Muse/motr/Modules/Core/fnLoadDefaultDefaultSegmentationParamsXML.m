function strctConfig = fnLoadDefaultDefaultSegmentationParamsXML(strConfigurationFile)
    ...
    if strcmpi(strctRoot.type,'element')
        if strcmpi(strctRoot.name,'Module')
            ...
            for iIter=1:length(strctRoot.attributes)
                ...
                if bNumeric
                    if length(afval) > 1
                        strCmd = [strHostVar,strctRoot.name,'.',strctRoot.attributes{iIter}.key,' = [',val,']; '];
                    else
                        strCmd = [strHostVar,strctRoot.name,'.',strctRoot.attributes{iIter}.key,' = ',val,'; '];
                    end;
                else
...
strctConfig=strctConfig.m_strctSegmentation;
