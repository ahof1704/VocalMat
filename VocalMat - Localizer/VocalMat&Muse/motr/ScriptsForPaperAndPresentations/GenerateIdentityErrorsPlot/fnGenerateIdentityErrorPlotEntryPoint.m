...
for iIter=1:length(astrctGroundTruth)
  % Find which frames were done by both annotators....
    ...
    abHuddling1 = astrctGroundTruth(iIter).m_abHuddling(abFrames1);
    abHuddling2 = astrctGroundTruth2(iIter).m_abHuddling(abFrames2);

    abFailedSeg1 = astrctGroundTruth(iIter).m_abFailedSeg(abFrames1);
    abFailedSeg2 = astrctGroundTruth2(iIter).m_abFailedSeg(abFrames2);

    abAgreeOnNotFail = ~abFailedSeg1 & ~abFailedSeg2;
    
    
    
    [a2iAssignment1] = fnGenerateIdentityErrorPlotAux(...
        astrctGroundTruth(iIter).m_astrctCorrectPosition, astrctGroundTruth(iIter).m_astrctTrackers, ...
        aiFrames1(abAgreeOnNotFail));

    
    [a2iAssignment2] = fnGenerateIdentityErrorPlotAux(...
        astrctGroundTruth2(iIter).m_astrctCorrectPosition, astrctGroundTruth2(iIter).m_astrctTrackers, ...
        aiFrames2(abAgreeOnNotFail));
    
    abDisagreeOnIdentity(iIter) = sum(all(a2iAssignment1 == a2iAssignment2,2) == 0);
    
    aiNumFrames(iIter) = length(aiSharedFrames);
    aiNotSameFailedSeg(iIter) = sum(abFailedSeg1 ~= abFailedSeg2);
    aiNotSameHuddling(iIter) = sum(abHuddling1 ~= abHuddling2);
