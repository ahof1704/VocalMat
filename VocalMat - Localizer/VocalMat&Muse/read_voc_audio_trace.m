...
% path_name=sprintf('%s/demux', exp_dir_name);  %Original
path_name=(exp_dir_name);
% base_name=sprintf('Test_%s_1',letter_str);  %Original
pindx=strfind(path_name,'\');
base_name=path_name(pindx(end)+1:end);
...
for i=1:n_signals
    ...
    fname=fullfile(path_name,[base_name '.WAV.ch' num2str(i)]);
