...
for i = 1:7
    ...
    elseif i == 2
        ...
        ytick_val = [0 0.06 0.12];
        yticklabel_l = {' ' ' 0.06 ' ' 0.12 '};
        ylim_val = [-0.0001 0.1201];
        ycolor_l = 'k';
    ...
    elseif i == 5
        ...
        ytick_val = [0 0.12 0.24];
        yticklabel_l = {' ' ' 0.12 ' ' 0.24 '};
        ylim_val = [-0.0001 0.2401];
        ycolor_l = 'w';
    ...
    elseif i == 6
        ...
        %just copied low not using data for poster
                xtick_val = [1 10 100 1000];
                xticklabel_l = {'1' '10' '100' '1000'};
%                 xticklabel_l = cellstr(num2str(round(log10(xtick_val(:)
                xlim_val = [0 1000];
        
        ytick_val = [0 0.04 0.08];
        yticklabel_l = {' ' ' 0.04 ' ' 0.08 '};
        ylim_val = [-0.0001 0.0801];
        ycolor_l = 'w';
    else
        tmp = ivi_ss;
        bin_size = 0.01;%1000th of a ms bins
    ...
    for j = 1:4%num sessions with vocs
        ...
        doi = tmp(1,ses_loc);
        h = figure;
        set(h,'position',[1   1   560   420]);
        ha(j) = gca;
        [n,xout] = hist(doi,binEdges);
        n = n/sum(data_set_count(animal,1));
        area(xout,n,'FaceColor',color_l,'EdgeColor','k')
    ...
    for j = 1:size(ha,2)
        ...
        if j == 1
            plot([m m],[0 ylim_val(2)],'r-.','LineWidth',2) % median
        else
            plot([m m],[0 ylim_val(2)],'k-.','LineWidth',2) % median
        end
        if i == 6 %j == 2 && i == 6%j == 2 && i == 5
            if j == 1
                hl = legend('  Combined','  Median');
            elseif j == 2
                hl = legend('  Mouse 1','  Median');
            elseif j == 3
                hl = legend('  Mouse 2','  Median');
            elseif j == 4
                hl = legend('  Mouse 3','  Median');
            end
            set(hl,'box','off','FontName','Arial','Location','NorthWest','FontSize',20);
        end
    end
    
    if isdir('A:\Neunuebel\method_paper\figures\figure2')==0
        mkdir('A:\Neunuebel\method_paper\figures\figure2')
    end
    cd 'A:\Neunuebel\method_paper\figures\figure2'
    for j = 1:size(ha,2)
        figure(j)
