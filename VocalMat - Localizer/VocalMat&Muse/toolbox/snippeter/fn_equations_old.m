...
%   microphones based on position of each moused
%
...
m = zeros(size(mouse,2),9);
for i = 1:size(mouse,2)
    x = mouse(i).x*meters_2_pixels;
    y = mouse(i).y*meters_2_pixels;
    z = 0*meters_2_pixels;
    ...
    m(i,5) = (sqrt((positions_out(2).x_m-x)^(2)+(positions_out(2).y_m-y)^(2)+(positions_out(2).z_m-z)^(2))-sqrt((positions_out(3).x_m-x)^(2)+(positions_out(3).y_m-y)^(2)+(positions_out(3).z_m-z)^(2)))/Vsound;
    %estimated delay between mic 2 vs mic4
    m(i,6) = (sqrt((positions_out(2).x_m-x)^(2)+(positions_out(2).y_m-y)^(2)+(positions_out(2).z_m-z)^(2))-sqrt((positions_out(4).x_m-x)^(2)+(positions_out(4).y_m-y)^(2)+(positions_out(4).z_m-z)^(2)))/Vsound;
    %estimated delay between mic 3 vs mic4
    m(i,9) = (sqrt((positions_out(3).x_m-x)^(2)+(positions_out(3).y_m-y)^(2)+(positions_out(3).z_m-z)^(2))-sqrt((positions_out(4).x_m-x)^(2)+(positions_out(4).y_m-y)^(2)+(positions_out(4).z_m-z)^(2)))/Vsound;
