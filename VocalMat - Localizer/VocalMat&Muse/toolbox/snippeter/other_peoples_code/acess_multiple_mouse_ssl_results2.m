...
if to_do<=3
    for i = 1:size(head_error,1)
        ...
        if strcmp(odd_voc,'n')==1
            ...
            elseif size(coords_chunks,2)==2
                centroid_chunks(i,1) = mean(X);
                centroid_chunks(i,2) = mean(Y);
                %                 [idx,dm,mm,C] = kur_rce(coords_chunks',0);
                %still need to figure out how to add var from one deminsion
                %to the next if only 2 points....this does not look
                %correct.
                C = cov(X,Y);
                C2 = C;
                [V,D] = eig(C);
                [val_C1 loc_min] = min(diag(D));
                [val_C2 loc_max] = max(diag(D));
                
                D(loc_min,loc_min) = D(loc_max,loc_max);
                D(2,2) = D(1,1);
                C = V*D*V';
                
%                 [val_C1 loc_min] = min(diag(C));
%                 [val_C2 loc_max] = max(diag(C));
%                 C(loc_min,loc_min) = C(loc_max,loc_max);
                [R,err] = cholcov(C,0);
