...
load('U:\Matlab\progs\v33_multiple_mice_cleaned_up\r_est_raw_for_06052012_D_voc_video_frame_chunk_DA7_pdfs.mat')
load('A:\Neunuebel\ssl_sys_test\sys_test_06052012\Data_analysis7\Test_D_1_Mouse.mat')
saving_dir = 'A:\Neunuebel\ssl_sys_test\sys_test_06052012\Data_analysis7\';
...
for i = 1:size(i_syl,2)
    ...
    char_value = voc_num(pos1+1:end);
    switch ischar(char_value)
        case strcmp(char_value,'0')==1
            col = 1;
            color_v = 'm';
        case strcmp(char_value,'a')==1
            col = 2;
            color_v = 'r';
        case strcmp(char_value,'b')==1
            col = 3;
            color_v = 'b';
        case strcmp(char_value,'c')==1
            col = 4;
            color_v = 'c';
        case strcmp(char_value,'d')==1
            col = 5;
            color_v = 'k';
        case strcmp(char_value,'e')==1
            col = 6;
            color_v = 'g';
    end
...
%removes vocs with total time less than min_seg_time
tmp_s = find(dur_mat(:,1)<min_seg_time);
head_error(tmp_s,:)=[];%removed any vocalizations that are only part of chunk1 and whole vocalization
dur_mat(tmp_s,:) = [];
clear tmp_s
...
for i = 0
    ...
    for j = 1:size(tmp,2)        
        ...
        plot(tmp(s:e,j),s:e,color_l,'MarkerFaceColor',color_f)
    end
end
close all
...
filename = 'Varibility_error_voc_frame_chunk';
cd (saving_dir)
print(hf(1),'-dpdf','-r600','-painters',filename);
for i=1:size(hf,2)
        file = sprintf('%s_%d.pdf',filename,i);
        print(hf(i),'-dpdf','-r600','-painters',file)
        clear file
end 