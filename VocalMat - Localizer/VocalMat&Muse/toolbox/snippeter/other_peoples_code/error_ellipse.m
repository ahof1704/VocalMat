function [h,size_space,return_x,return_y] =error_ellipse(varargin)
...
if plot_on == 1
    hold_state = get(gca,'nextplot'); %changed on 5/2/2013
end
if r==3 & c==3
    z0=mu(3);
    ...
    alpha(0.3)
    camlight
    volume = prod(sqrt(eig(C)))*4/3*pi*k; %added on 04072013 by jpn to calculate volume of ellipsiod
    if nargout
        h=[h1 h2 h3 h4];
        size_space = volume; %added on 04072013 by jpn to return volume of ellipse
    end
elseif r==2 & c==2
    % Make the matrix has positive eigenvalues - else it's not a valid covariance matrix!
    if any(eig(C) <=0)
        ...
        area=prod(sqrt(eig(C)))*pi*k; %added on 04072013 by jpn to calculate area of ellipse
        if nargout
            h=[];%h1;
            size_space = area;%added on 04072013 by jpn to return area of ellipse
            return_x = x;
            return_y = y;
        end
    end
...
%axis equal
if plot_on == 1
    set(gca,'nextplot',hold_state);%changed on 5/2/2013
end
%---------------------------------------------------------------
