...
if mod(i,2) %Only enters here every two sheets
    ...
    for animal = 0:1 %Two animals per tracking sheet
        for j=1:size(alldata{1,i+animal},1)
            small_time = abs(alldata{1,i+animal}(j,5)-alldata_xy{1,xy_file}(:,1));
            [min_time,index] = min(small_time);
            alldata{1,i+animal}(j,13:17) = alldata_xy{1,xy_file}(index,4:8);
        end
    end
        ...
        if ~isempty(C)
            for j = 1:size(index_a,1)
                if abs((alldata{1,i}(index_a(j),5))-(alldata{1,i+1}(index_b(j),5))) < min_diff_time %Verify if they are close enough in time.
                     if abs(alldata{1,i+1}(index_b(j),7)-alldata{1,i}(index_a(j),7)) < min_diff_intensity %Compare the intensity and if they are simmilar, it is noise.
    %                     if abs(alldata{1,i}(index_a(j),12) - alldata{1,i+1}(index_b(j),12)) < min_diff_freq %Verify if the fundamental freq is also equal doesn't work. The noise can present huge difference in frequency.
                            disp(['Vocalization' ]);
                            alldata{1,i}(index_a(j),:) = NaN;   %Eliminates the noise in both tables
                            alldata{1,i+1}(index_b(j),:) = NaN;
                        ...
                        if alldata{1,i+1}(index_b(j),7) > alldata{1,i}(index_a(j),7)   %Who has greater intensity?
                            [max_val,where_mom] = max(alldata{1,i+1}(index_b(j),13:15)); %Center = 13; Left = 14; Right 15
                            if where_mom==2 %The mom was in opposite side (Pup who made the vocalization is on the right side and the mom was on the left)
                                pup_right = [pup_right;  alldata{1,i+1}(index_b(j),:)];
                                alldata{1,i+1}(index_b(j),:) = NaN; %I can't really remove (like =[]) because I get a list of overlapping at the beginning of this loop and I cant shift the indexes there.
                            else
                                unknown = [unknown; alldata{1,i+1}(index_b(j),:)];
                                alldata{1,i+1}(index_b(j),:) = NaN;
                            end
                        else
                            [max_val,where_mom] = max(alldata{1,i}(index_a(j),13:15));
                            if where_mom==3 %The mom was in opposite side
                                pup_left = [pup_left;  alldata{1,i}(index_a(j),:)];
                                alldata{1,i}(index_a(j),:) = NaN; %Remove this vocalization from the list
                            else
                                unknown = [unknown; alldata{1,i}(index_a(j),:)];
                                alldata{1,i}(index_a(j),:) = NaN;
                            end
                        end
                    end
                end
    ...
    mom_right = find(alldata{1,i}(:,15)==1); %Center = 13; Left = 14; Right = 15
    for temp = mom_right' 
        pup_left = [pup_left; alldata{1,i}(temp,:)]; %Shouldn't be necessary to remove these vocalizations from the original list... 
        alldata{1,i}(temp,:) = NaN; %But I will do just to see if the alldata ends up empty at the end of the process.
    end
    
    mom_left = find(alldata{1,i+1}(:,14)==1);
    for temp = mom_left' 
        pup_right = [pup_right; alldata{1,i+1}(temp,:)];
        alldata{1,i+1}(temp,:) = NaN;
    end
...
%        figure
       [y2,x2] = ecdf(pup_left(:,12));
       [y1,x1] = hist((pup_left(:,12)),n_points,'BinLimits',[40000,110000]); 
       subplot(2,2,1),plotyy(x1,y1,x2,y2,@(x,y)bar(x,y,1,'c'),'stairs')
       title(['Frequency distribution (Left Pup) ' sheetname{i}]);
...
%        subplot(2,2,1), hist((pup_left(:,12)),n_points,'BinLimits',[40000,110000]); title(['Frequency distribution (Left Pup) ' sheetname{i}]);
       ...
       [y2,x2] = ecdf(pup_left(:,2));
       [y1,x1] = hist((pup_left(:,2)),n_points,'BinLimits',[40000,110000]); 
       subplot(2,2,3),plotyy(x1,y1,x2,y2,@(x,y)bar(x,y,1,'c'),'stairs')
       title(['Duration distribution (Left Pup) ' sheetname{i}]);
...
%        subplot(2,2,3), hist((pup_left(:,2)),n_points); title(['Duration distribution (Left Pup) ' sheetname{i}]);
       ...
       [y2,x2] = ecdf(pup_right(:,12));
       [y1,x1] = hist((pup_right(:,12)),n_points,'BinLimits',[40000,110000]);
       subplot(2,2,2),plotyy(x1,y1,x2,y2,@(x,y)bar(x,y,1,'c'),'stairs')
       title(['Frequency distribution (Right Pup) ' sheetname{i+1}]);
...
%        subplot(2,2,2), hist((pup_right(:,12)),n_points,'BinLimits',[40000,110000]); title(['Frequency distribution (Right Pup) ' sheetname{i+1}]);
       ...
       [y2,x2] = ecdf(pup_right(:,2));
       [y1,x1] = hist((pup_right(:,2)),n_points,'BinLimits',[40000,110000]); 
       subplot(2,2,4),plotyy(x1,y1,x2,y2,@(x,y)bar(x,y,1,'c'),'stairs')
       title(['Duration distribution (Right Pup)' sheetname{i+1}]);
...
%            ],'VerticalAlignment','top','HorizontalAlignment','left')
    ...
    %    saveas(gcf,sheetname{i},'jpg')
       ...
       xlswrite([vpathname 'All_pups'],pup_left,[sheetname{i} '_Left'])
       xlswrite([vpathname 'All_pups'],pup_right,[sheetname{i+1} '_Right'])
...
figure('Name',['All Data_' vfilename],'NumberTitle','off','units','normalized','outerposition',[0 0 1 1])
 ...
 [y2,x2] = ecdf(alldata_total(:,12));
 [y1,x1] = hist((alldata_total(:,12)),n_points,'BinLimits',[40000,110000]); title('Frequency distribution');
...
ylabel('Occurrences');
 ...
 [y2,x2] = ecdf(alldata_total(:,2));
 [y1,x1] = hist((alldata_total(:,2)),n_points,'BinLimits',[40000,110000]); title('Duration distribution');
