...
%     strcmp(txt{1,1},'Control')
    ...
    list = {list(1).name,list(2).name,list(3).name,list(4).name};
    ...
    list = {list(3).name,list(4).name,list(1).name,list(2).name};
end
...
%         if mod(i,2) %Only enters here every two sheets
            ...
            figure('Name',[list{i} ' and ' list{i+2}],'NumberTitle','off','units','normalized','outerposition',[0 0 1 1])
            %        figure
