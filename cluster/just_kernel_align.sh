...
#while read p; do
#  OUT0=$(sbatch -p scavenge --requeue --parsable -J $p -o $p.stdout.txt -e $p.stderr.txt /ysm-gpfs/project/ahf38/Antonio_VocalMat/VocalMat-Identifier/VocalMat_Full.sh ${p} ${folder_name})
...
OUT2=$(sbatch -p scavenge --requeue  -J kerner_alignment -o kernel_aligment.stdout.txt -e kernel_aligment.stderr.txt /ysm-gpfs/project/ahf38/Antonio_VocalMat/end.sh ${folder_name})
