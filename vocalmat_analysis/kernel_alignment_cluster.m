...
if group_animals==1
    ...
    for i=1:length(keyword)
       ...
       eval(['list{i} = dir(fullfile(work_dir, [''*'' keyword{' num2str(i) '} ''*'']));']);
       isdir = [list{i}.isdir].';
       list_dir = list{i}(isdir,:); list{i} = list_dir;
       list{i}.name
    end

    for i=1:length(list)
        isdir = [list{i}.isdir].';
        list_dir = list{i}(isdir,:); %list_dir(1:2)=[];
        map=[];label=[];
        disp(['[vocalmat]: Combining ' keyword{i} ' files'])
        T_classProb_temp =[];
        for j=1:size(list_dir,1)
            try
                disp([' |-' list_dir(j).name])
                load(fullfile(work_dir, list_dir(j).name,'T_classProb.mat'))
                T_classProb_temp = [T_classProb_temp; T_classProb];
            catch
                disp([' |- Problem with ' list_dir(j).name])
            end
        end
        T_classProb = T_classProb_temp;
        disp(['[vocalmat]: starting VocalMat Analsyis...'])
        mkdir(['maps_' keyword{i}])
        vfilename = keyword{i}; vfile = fullfile(work_dir, ['maps_' keyword{i}]);
        run(fullfile(analysis_path,'diffusion_maps_cluster'))
    end
end
...
for list_idx = 1:size(list,1)-1
    ...
    for list2_idx= list_idx+1:size(list,1)
        try
            ...
            disp([name ' vs ' name2])
            try
                data = load(fullfile(work_dir, name, ['maps_' name])); eval(['X1 = data.maps_' name ';']); X1 = double(X1); X1_orig=X1'; 
                eval(['Y1 = data.label_maps_' name ';']); Y1 = double(Y1); Y1_orig = Y1;
            catch
                data = load(fullfile(work_dir, name, name)); eval(['X1 = data.' name ';']); X1 = double(X1); X1_orig=X1'; 
                eval(['Y1 = data.label_' name ';']); Y1 = double(Y1); Y1_orig = Y1;  
            end
            try
                data2 = load(fullfile(work_dir, name2, ['maps_' name2])); eval(['X2 = data2.maps_' name2 ';']); X2 = double(X2); X2_orig=X2'; 
                eval(['Y2 = data2.label_maps_' name2 ';']); Y2 = double(Y2); Y2_orig = Y2;
            catch
                data2 = load(fullfile(work_dir , name2 , name2)); eval(['X2 = data2.' name2 ';']); X2 = double(X2); X2_orig=X2'; 
            ...
            if ~exist(fullfile(path_to_save, [X1_group '_' X2_group]))
                ...
                path_to_save = fullfile(path_to_save, [X1_group '_' X2_group]) ;
            else
                mkdir(fullfile(path_to_save, [X1_group '_' X2_group]))
                path_to_save = fullfile(path_to_save, [X1_group '_' X2_group]);
            end
		...
		disp(['[vocalmat]: |-KNN graph number of neighbors'])
                r1 = []; rT1 = []; r2 = []; rT2 = [];
                rl1 = []; rlT1 = []; rl2 = []; rlT2 = [];
                rW1 = []; rWT1 = []; rW2 = []; rWT2 = [];
...
%                 disp('Mapping with Wang11 method...')
		...
		disp(['[vocalmat]: |-Graph Laplacian'])
                ...
                Sw = sum(sum(W));
                Ws = Ws/Sws*Sw;
	        disp(['[vocalmat]: |-Finished normalization'])
	        ...
	        disp(['[vocalmat]: |-Finished eigendecomposition'])
                ...
                mean(ErrRec);
                mean(ErrRecInv);
	        disp(['[vocalmat]: |-Finished estimating error'])
	        ...
	        disp(['[vocalmat]: |-Finished projection'])
            ...
            if min(size(XT1,1),size(XT2,1)) == 2
                ...
                if show_plots ==1
                    ...
                    view(-50, 10);
                    set(gca,'fontsize', 20);
                    saveas(gcf,fullfile(path_to_save, ['orig&domains_' X1_group '_' X2_group '.fig']))
                end
            end
            ...
            if show_plots==1
                ...
                set(gca,'fontsize', 15);
                saveas(gcf,fullfile(path_to_save, ['error_rate_1st_domain_' X1_group '_' X2_group '.fig']))
                ...
                % axis([0 nVectRBF 0 100])
                set(gca,'fontsize', 15);
                saveas(gcf,fullfile(path_to_save, ['error_rate_2nd_domain_' X1_group '_' X2_group '.fig']))
            end
            ...
            if rbf_k==1
                ...
                axis([-2.5 2.5 -2.5 2.5])
                view(-50, 10);
                saveas(gcf,fullfile(path_to_save, ['projected_rbf_domains_' X1_group '_' X2_group '.fig']))
            end
            ...
            if wang==1 && show_plots ==1
                ...
                set(gca, 'TickLabelInterpreter', 'none');
                view(-50, 10);
                saveas(gcf,fullfile(path_to_save, ['projected_Wang_' X1_group '_' X2_group '.fig']))
            end
            ...
            if wang==1 && show_plots ==1
                ...
                set(gca,'fontsize', 15);
                view(-50, 10);
                saveas(gcf,fullfile(path_to_save, ['projected_best_align_Wang_' X1_group '_' X2_group '.fig']))
            end
            ...
            if wang==1 && show_plots ==1
                ...
                set(gca,'fontsize', 15);
                view(-50, 10);
                saveas(gcf,fullfile(path_to_save, ['projected_whole_data_best_align_Wang_' X1_group '_' X2_group '.fig']))
            end
            ...
            save(fullfile(path_to_save,[X1_group '_' X2_group]))
