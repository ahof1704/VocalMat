...
for current_model=1:models_found
    ...
    for current_dataset=1:datasets_for_this_model
        ...
        if current_dataset == 1
            ...
            models{current_model,1}(1,5)  = "correct3";
            models{current_model,1}(1,6)  = "wrong1";
            models{current_model,1}(1,7)  = "wrong2";
            models{current_model,1}(1,8)  = "wrong3";
            models{current_model,1}(1,9)  = "top1";
            models{current_model,1}(1,10) = "top2";
            models{current_model,1}(1,11) = "top3";
            models{current_model,1}(1,12) = "chevron_accuracy";
            models{current_model,1}(1,13) = "complex_accuracy";
            models{current_model,1}(1,14) = "down_fm_accuracy";
            models{current_model,1}(1,15) = "flat_accuracy";
            models{current_model,1}(1,16) = "mult_steps_accuracy";
            models{current_model,1}(1,17) = "noise_dist_accuracy";
            models{current_model,1}(1,18) = "rev_chevron_accuracy";
            models{current_model,1}(1,19) = "short_accuracy";
            models{current_model,1}(1,20) = "step_down_accuracy";
            models{current_model,1}(1,21) = "step_up_accuracy";
            models{current_model,1}(1,22) = "two_steps_accuracy";
            models{current_model,1}(1,23) = "up_fm_accuracy";
        end
        ...
        try
            ...
            top2       = correct2*100/num_images;
                            ...
                            % -- move things around to get top3 statistics
                            failed3 = ~correct_top2;
                            Ttop3 = Twrong(failed3,:);
                            ...
                            % -- slow again
                            for current_highest=1:size(idx_highest,1)
                                Ttop3.Top3(current_highest) = string(scores_labels(1, idx_highest(current_highest)));
                            end
                            Twrong3 = Ttop3;

                            Tlabel = table2cell(Ttop3(:,2));
                            Tlabel = categorical(Tlabel);

                            Tpred  = table2cell(Ttop3(:,end));
                            Tpred  = categorical(string(Tpred));

                            correct_top3 = Tlabel == Tpred;
                            Ttop3        = Twrong(correct_top3,:);

                            % -- sort scores and get ratio for top2 accurary
                            % scores        = table2array(Ttop3(:,5:16));
                            % sorted_scores = sort(scores,2);
                            % ratio         = 1 - (sorted_scores(:,end-2)./sorted_scores(:,end));
                            % ratios{current_model,1}{current_dataset,1} = ratio;
                            % scatter(1:1:size(ratio,1), ratio);

                            % -- top2 statistics
                            correct3     = sum(strcmp(Twrong3.Training_label, Twrong3.Top3));
                            correct3     = correct2 + correct3;
                            wrong3       = size(T,1) - correct3;
                            top3         = correct3*100/num_images;

                            Tlabel       = table2cell(Twrong3(:,2));
                            Tlabel       = categorical(Tlabel);

                            Tpred        = table2cell(Twrong3(:,end));
                            Tpred        = categorical(string(Tpred));
                            correct_top3 = Tlabel == Tpred;
                            Ttop3        = Twrong3(correct_top3,:);

                            scoresw3 = table2array(Twrong3(:,5:end-2));
                            scoresw3 = sort(scoresw3, 2);
                            sominha3w(:,1) = scoresw3(:,end) + scoresw3(:,end-1);
                            sominha3w(:,2) = scoresw3(:,end) + scoresw3(:,end-1) + scoresw3(:,end-2);

                            scoresc3 = table2array(Ttop3(:,5:end-2));
                            scoresc3 = sort(scoresc3, 2);
                            sominha3c(:,1) = scoresc3(:,end) + scoresc3(:,end-1);
                            sominha3c(:,2) = scoresc3(:,end) + scoresc3(:,end-1) + scoresc3(:,end-2);
            ...
            models{current_model,1}(current_dataset+1,5)  = num2str(correct3);
            models{current_model,1}(current_dataset+1,6)  = num2str(wrong1);
            models{current_model,1}(current_dataset+1,7)  = num2str(wrong2);
            models{current_model,1}(current_dataset+1,8)  = num2str(wrong3);
            models{current_model,1}(current_dataset+1,9)  = num2str(top1);
            models{current_model,1}(current_dataset+1,10) = num2str(top2);
            models{current_model,1}(current_dataset+1,11) = num2str(top3);
            models{current_model,1}(current_dataset+1,12) = num2str(chevron_accuracy);
            models{current_model,1}(current_dataset+1,13) = num2str(complex_accuracy);
            models{current_model,1}(current_dataset+1,14) = num2str(down_fm_accuracy);
            models{current_model,1}(current_dataset+1,15) = num2str(flat_accuracy);
            models{current_model,1}(current_dataset+1,16) = num2str(mult_steps_accuracy);
            models{current_model,1}(current_dataset+1,17) = num2str(noise_dist_accuracy);
            models{current_model,1}(current_dataset+1,18) = num2str(rev_chevron_accuracy);
            models{current_model,1}(current_dataset+1,19) = num2str(short_accuracy);
            models{current_model,1}(current_dataset+1,20) = num2str(step_down_accuracy);
            models{current_model,1}(current_dataset+1,21) = num2str(step_up_accuracy);
            models{current_model,1}(current_dataset+1,22) = num2str(two_steps_accuracy);
            models{current_model,1}(current_dataset+1,23) = num2str(up_fm_accuracy);
            ...
            models{current_model,1}(current_dataset+1,21) = "table_performance.mat not found";
            models{current_model,1}(current_dataset+1,22) = "table_performance.mat not found";
            models{current_model,1}(current_dataset+1,23) = "table_performance.mat not found";
        end
    end
end
